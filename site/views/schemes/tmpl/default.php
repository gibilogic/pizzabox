...
/**
 * @version			  $Id: views/schemes/tmpl/default.php 2012-08-18 22:50:00Z zanardi $
...
?>
<h1><?php echo JText::_('PIZZABOX_SCHEME_SELECT') ?></h1>
<form action="index.php?option=com_pizzabox&controller=schemes" method="post" name="schemeForm" id="schemeForm">
  <div class="buttons">
    <a class="btn" href="index.php?option=com_pizzabox&controller=containers"><?php echo JText::_('JPREV') ?></a>
    <input class="btn btn-primary" type="submit" value="<?php echo JText::_('JNEXT') ?>" />
    <input type="hidden" name="task" value="save" /> 
    <input type="hidden" name="element" value="scheme" />
  ...
  <div id="radiobox-wrapper" class="clearfix">
    <?php foreach ($this->rows as $k => $row) :?>
      <div class="radiobox radio_<?php echo $row->id ?>">
        <input type="radio" id="radio_<?php echo $row->id ?>" name="schemes" value="<?php echo $row->id ?>"/>
