...

{
    ...
    $this->helper = new PizzaboxHelper();
		...
		if ( $tpl == 'delivery_name' ) {
			...
			$this->assignRef('user_id', JFactory::getUser()->id);
      ...
      $this->order_total = $orderModel->getTotal();
		...
		$this->addTemplatePath( JPATH_COMPONENT.DS.'layouts' );
		$this->setLayout('');
  ...
  // or that we have enough time to prepare the order (in Preparation Mode)
	function getSessionTimesList( $start_time, $end_time, $interval, $delivery_date )
	{
  ...
  function getMinStartTime()
  {
	...
	function getElementImage ( $elements_type, $id ) {
